import React from 'react';

type Page = 'home' | 'oracle';

interface HeaderProps {
  navigateTo: (page: Page, hash?: string) => void;
}

const Header: React.FC<HeaderProps> = ({ navigateTo }) => {
  const navItems: { label: string, action: () => void }[] = [
    { label: 'Início', action: () => navigateTo('home') },
    { label: 'Oráculo', action: () => navigateTo('oracle') },
    { label: 'Orixás', action: () => navigateTo('home', '#orixas') },
    { label: 'Depoimentos', action: () => navigateTo('home', '#depoimentos') },
    { label: 'Comprar Axés', action: () => navigateTo('home', '#cta') },
    { label: 'Contato', action: () => navigateTo('home', '#contato') },
  ];

  return (
    <header className="w-full max-w-6xl text-center py-8 px-4 sticky top-0 z-50 bg-ase-deep-blue/80 backdrop-blur-md">
      <div className="flex justify-between items-center">
        <div className="text-left cursor-pointer" onClick={() => navigateTo('home')}>
            <h1 className="text-4xl font-serif font-bold text-ase-gold">Àlá Àse</h1>
            <p className="text-md text-gray-300">Oráculos de Axé</p>
        </div>
        <nav>
          <ul className="flex justify-center space-x-2 md:space-x-6">
            {navItems.map(({ label, action }) => (
              <li key={label}>
                <button
                  onClick={action}
                  className={`py-2 px-1 md:px-2 text-sm md:text-base font-semibold transition-colors duration-300 border-b-2 border-transparent text-gray-300 hover:text-ase-gold hover:border-ase-gold`}
                >
                  {label}
                </button>
              </li>
            ))}
          </ul>
        </nav>
      </div>
    </header>
  );
};

export default Header;